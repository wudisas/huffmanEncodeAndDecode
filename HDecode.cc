...
void getHuffmanChar(const Node* asciiTrieNode, int& result){
	while(true){
		if(asciiTrieNode->leftkid == NULL) {
			result = asciiTrieNode->asciiValue;
			return;
		}
		int tmp = getchar();
		if(tmp == -1) throw "DONE";
		if(tmp == 48) asciiTrieNode = asciiTrieNode->leftkid;
		else if(tmp == 49) asciiTrieNode = asciiTrieNode->rightkid;
	}
}
	...
	try{
		while(true){
			...
			forEach(i,0,charNeed){
				charCount++;
				int target;
				getHuffmanChar(asciiTrieRoot, target);
				string s(1,target);
				cout<<s;
				asciiNodes[target]->update(++asciiNodes[target]->frequency); //update weight and frequency
			}
